# 🗄️ Data Architecture: recruitment-service
...
| **Connection URL** | `jdbc:h2:mem:recruitment_service` |
...
3. Access H2 Console: `http://localhost:8084/h2-console`
   - JDBC URL: `jdbc:h2:mem:recruitment_service`
...
fractalx:
  modules:
    ...
    recruitment-service:
      datasource:
        url: jdbc:mysql://localhost:3306/recruitment_service_db
        username: root
...
### `hire-employee-saga` — **Owner** 👑

> Hires a candidate: creates employee record and updates department headcount.

This service **owns** this saga. The `@DistributedSaga` annotation was found in `RecruitmentModule.hireEmployee()` and the orchestrator service was auto-generated from it.
...
**Payload fields** (`HireEmployeeSagaPayload`):
...
1. `employee-service` → `createEmployee()`  ↩ compensate: `cancelCreateEmployee()`
2. `department-service` → `incrementHeadcount()`  ↩ compensate: `cancelIncrementHeadcount()`
