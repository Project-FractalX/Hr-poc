...
public class RecruitmentController {
    ...
    public RecruitmentController(RecruitmentModule recruitmentModule, RecruitmentService recruitmentService) {
    ...
    public ResponseEntity<CandidateResponse> apply(@Valid @RequestBody ApplyRequest req) {
        return ResponseEntity.ok(CandidateResponse.from(recruitmentService.apply(req.getFirstName(), req.getLastName(), req.getEmail(), req.getPosition(), req.getDepartmentId(), req.getExpectedSalary())));
    }
    ...
    public ResponseEntity<CandidateResponse> hire(@Valid @RequestBody HireCandidateRequest req) {
        return ResponseEntity.ok(CandidateResponse.from(recruitmentModule.hireEmployee(req.getCandidateId(), req.getFirstName(), req.getLastName(), req.getEmail(), req.getPosition(), req.getSalary(), req.getDepartmentId())));
    }
    ...
    public ResponseEntity<List<CandidateResponse>> shortlisted() {
        return ResponseEntity.ok(recruitmentService.findShortlisted().stream().map(CandidateResponse::from).collect(Collectors.toList()));
    }
}
