...
public class LeaveController {
    ...
    public ResponseEntity<LeaveResponse> approve(@Valid @RequestBody ApproveLeaveRequest req) {
        LeaveEntry entry = leaveModule.approveLeave(
            req.getEmployeeId(), req.getLeaveType(),
            req.getStartDate(), req.getEndDate(),
            req.getLeaveDays(), req.getReason());
    ...
    public ResponseEntity<List<LeaveResponse>> getByEmployee(@PathVariable Long employeeId) {
        return ResponseEntity.ok(leaveService.findByEmployee(employeeId).stream()
            .map(LeaveResponse::from).collect(Collectors.toList()));
    }
