...
public class DepartmentController {
    ...
    public ResponseEntity<DepartmentResponse> create(@Valid @RequestBody CreateDepartmentRequest req) {
        return ResponseEntity.ok(DepartmentResponse.from(
            departmentService.createDepartment(req.getCode(), req.getName(), req.getAnnualBudget())));
    }
    ...
    public ResponseEntity<List<DepartmentResponse>> listActive() {
        return ResponseEntity.ok(departmentService.findAllActive().stream()
