...
public class PayrollController {
    ...
    public ResponseEntity<List<PayrollResponse>> getByEmployee(@PathVariable Long employeeId) {
        return ResponseEntity.ok(payrollService.findByEmployee(employeeId).stream()
            .map(PayrollResponse::from).collect(Collectors.toList()));
    }

    @GetMapping("/period/{year}/{month}")
    public ResponseEntity<List<PayrollResponse>> getByPeriod(
            @PathVariable Integer year, @PathVariable Integer month) {
        return ResponseEntity.ok(payrollService.findByPeriod(year, month).stream()
            .map(PayrollResponse::from).collect(Collectors.toList()));
    }
}
