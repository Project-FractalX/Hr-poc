...
public class EmployeeController {
    ...
    public ResponseEntity<EmployeeResponse> onboard(@Valid @RequestBody OnboardEmployeeRequest req) {
        Employee e = employeeModule.onboardEmployee(
            req.getFirstName(), req.getLastName(), req.getEmail(),
            req.getPosition(), req.getSalary(), req.getDepartmentId());
    ...
    public ResponseEntity<List<EmployeeResponse>> listActive() {
        return ResponseEntity.ok(employeeService.findActive().stream()
    ...
    public ResponseEntity<List<EmployeeResponse>> getByDepartment(@PathVariable Long departmentId) {
        return ResponseEntity.ok(employeeService.findByDepartment(departmentId).stream()
            .map(EmployeeResponse::from).collect(Collectors.toList()));
    }
